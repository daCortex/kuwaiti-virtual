...
export default async function FilePage() {
  ...
  if (!d) {
    return (
      <section className="mx-auto max-w-md px-6 py-32 text-center">
        ...
        <Link href="/crew" className="mt-6 inline-flex rounded-full bg-gold px-6 py-3 text-sm font-medium text-white">Sign in</Link>
      </section>
    );
  }

  // Merge in staff-added aircraft + multipliers (Crew Center → Aircraft · Multipliers).
  await refreshFleetOps();
  const staffAc = getStaffAircraft();
  const groups = staffAc.length ? [...AIRCRAFT_GROUPS, { group: "Added by staff", items: staffAc }] : AIRCRAFT_GROUPS;
  const multipliers = [...MULTIPLIERS, ...getStaffMultipliers()];
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-10 lg:px-8">
      ...
      <FilePirep groups={groups} multipliers={multipliers} />
    </div>
  );
}
